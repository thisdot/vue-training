<template>
  <div></div>
</template>

<script>
// #Exercise 4
// Summary: This exercise is aimed at introducing you to vuex
// Story: An an user I would like to have all my jokes being handled and stored in my vuex store.
//        I would also like to have the ability to "add" new jokes.

// This exercise will feature:
// - Vuex module
// - use of getter / mutation / action and state
// - Vuex Integration in the component

//Steps
// - Create a new file within the store/modules folder
// - Load the new module within the store/index.js
// - Create an action / mutation / state to fetch the jokes
// - Change the ExerciseTwo to trigger the action
// - Change exerciseTwo to load jokes from the store
// - Use this file to create a simple component to "ADD jokes"
// - Call required Vuex Action to "add" joke to the state

//EXTRA
// - Use v-model with vuex to add a new joke
// - Add a watched in the ExerciseTwo to trigger a "log" when Jokes changes
// - Change the values in the chrome/firefox debugger
</script>

<style></style>
